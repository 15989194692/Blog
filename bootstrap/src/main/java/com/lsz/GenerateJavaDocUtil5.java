...
public class GenerateJavaDocUtil5 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 将提取的内容写入Markdown文档
            try (FileWriter writer = new FileWriter("SampleApi_API_Documentation5.md")) {
    ...
    private static void parsePojo(Type classType, StringBuilder sb) {
        try {
            List<Type> needParseTypes = new ArrayList<>();
            ...
            needParseTypes.stream().forEach(type -> parseType(type, sb));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
